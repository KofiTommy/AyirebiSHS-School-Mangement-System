...
if (!function_exists('logExternalSystemChange')) {
    function logExternalSystemChange($con, $actorName, $actorType, $actionType, $details = '', $targetUserId = '')
    {
        if (!$con || trim((string)$actionType) === '') {
            return false;
        }
        ...
        @mysqli_stmt_bind_param($stmt, "ssssssss", $actorUserId, $actorName, $actorType, $actionType, $targetUserId, $details, $pageName, $ipAddress);
        $ok = @mysqli_stmt_execute($stmt);
        @mysqli_stmt_close($stmt);
        return $ok ? true : false;
    }
