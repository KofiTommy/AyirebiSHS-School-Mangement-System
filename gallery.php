...
<body>
  ...
  <main><section class="full-gallery" aria-labelledby="gallery-title"><div class="shell"><div class="gallery-title"><div><p class="eyebrow">The AYISEC gallery</p><h1 id="gallery-title">Our school<br><em>come alive.</em></h1></div><p>Explore the people, achievements, traditions, events and everyday life that make Ayirebi Senior High School special.</p></div><div class="gallery-wall"><?php foreach ($galleryImages as $image): $name = pathinfo($image, PATHINFO_FILENAME); $label = ucwords(str_replace(array('-', '_'), ' ', $name)); ?><button class="gallery-item" type="button" data-image="<?php echo htmlspecialchars($image, ENT_QUOTES, 'UTF-8'); ?>" data-label="<?php echo htmlspecialchars($label, ENT_QUOTES, 'UTF-8'); ?>"><img src="<?php echo htmlspecialchars($image, ENT_QUOTES, 'UTF-8'); ?>" alt="Ayirebi Senior High School — <?php echo htmlspecialchars($label, ENT_QUOTES, 'UTF-8'); ?>" loading="lazy"><span><?php echo htmlspecialchars($label, ENT_QUOTES, 'UTF-8'); ?></span></button><?php endforeach; ?></div></div></section></main>
