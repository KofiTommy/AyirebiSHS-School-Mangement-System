...
function lt_redirect_with_filters($batchId, $termName, $classId){
...
if(isset($_GET['delete_lesson']) && trim((string)$_GET['delete_lesson']) !== ''){
    ...
    lt_redirect_with_filters($filterBatchId, $filterTermName, $filterClassId);
}
...
if(isset($_POST['save_lesson_timetable'])){
    ...
    if($form['batchid'] === '' || $form['termname'] === '' || $form['classid'] === '' || $form['weekday'] === '' ||
       $form['subjectid'] === '' || $form['teacherid'] === '' || $form['starttime'] === '' || $form['endtime'] === ''){
        $flashMessage = lt_flash('warning', 'Please complete the class, semester, day, teacher, subject, and time fields.');
    ...
    } elseif(!lesson_timetable_teacher_subject_is_valid($con, $form['teacherid'], $form['subjectid'], $form['classid'], $form['batchid'], $form['termname'])){
        $flashMessage = lt_flash('error', 'That teacher is not currently assigned to teach the selected subject for this class, batch, and semester. Please update Subject Assignment first.');
    } elseif(lesson_timetable_has_class_overlap($con, $form['classid'], $form['batchid'], $form['termname'], $form['weekday'], $form['starttime'], $form['endtime'], $form['lessonid'])){
        $flashMessage = lt_flash('error', 'That class already has another lesson scheduled during the selected time.');
    } elseif(lesson_timetable_has_teacher_overlap($con, $form['teacherid'], $form['weekday'], $form['starttime'], $form['endtime'], $form['lessonid'])){
        ...
        if($form['lessonid'] !== ''){
            ...
            $saved = mysqli_query($con, "INSERT INTO tbllessontimetable(
                    lessonid,classid,batchid,termname,weekday,subjectid,teacherid,starttime,endtime,location,note,status,datetimeentry,updatedat,recordedby
                ) VALUES(
                    '$lessonIdEsc','$classIdEsc','$batchIdEsc','$termValue','$weekdayEsc','$subjectIdEsc','$teacherIdEsc','$startTimeEsc','$endTimeEsc','$locationEsc','$noteEsc','active',NOW(),NOW(),'$recordedByEsc'
                )");
            $_SESSION['LESSON_TIMETABLE_MESSAGE'] = $saved
                ? lt_flash('success', 'Lesson timetable saved successfully.')
                : lt_flash('error', 'The lesson timetable entry could not be saved.');
        }
        lt_redirect_with_filters($filterBatchId !== '' ? $filterBatchId : $form['batchid'], $filterTermName !== '' ? $filterTermName : $form['termname'], $filterClassId !== '' ? $filterClassId : $form['classid']);
    }
}
...
$assignmentOptions = lesson_timetable_fetch_assignment_options($con, $form['batchid'], $form['termname'], $form['classid']);
...
<main class="lesson-shell">
    ...
    <section class="lesson-toolbar lesson-print-hide">
        ...
        <a class="lesson-btn lesson-btn--secondary" href="lesson-timetable-report.php?batchid=<?php echo urlencode($filterBatchId); ?>&termname=<?php echo urlencode($filterTermName); ?>&classid=<?php echo urlencode($filterClassId); ?>"><i class="fa fa-eye"></i> Open Print Report</a>
    </section>
    ...
    <div class="lesson-grid lesson-grid--planner">
        <section class="lesson-card lesson-card--sticky">
            <div class="lesson-card__header">
                ...
                <a class="lesson-btn lesson-btn--secondary lesson-print-hide" href="lesson-timetable.php?batchid=<?php echo urlencode($filterBatchId); ?>&termname=<?php echo urlencode($filterTermName); ?>&classid=<?php echo urlencode($filterClassId); ?>"><i class="fa fa-times"></i> Cancel Edit</a>
                <?php } ?>
            </div>
        ...
        <section class="lesson-card">
            <div class="lesson-card__header">
                <div>
                    ...
                    <p>Focus the board by batch, semester, and class, then use the live week view below to make fast adjustments.</p>
                </div>
                ...
                <div class="lesson-helper-list lesson-helper-list--compact">
                    <?php if(count($assignmentOptions) > 0){ ?>
                        ...
                        <div class="lesson-helper-item lesson-helper-item--tinted" style="--lesson-accent: <?php echo lesson_timetable_escape($tokens['accent']); ?>; --lesson-surface: <?php echo lesson_timetable_escape($tokens['surface']); ?>;">
                            ...
                            <span><?php echo lesson_timetable_escape($option['teacher_name']); ?> handles <?php echo lesson_timetable_escape($option['class_name']); ?> in semester <?php echo lesson_timetable_escape($option['termname']); ?>, <?php echo lesson_timetable_escape($option['batch']); ?>.</span>
                        </div>
                        <?php } ?>
                    <?php } else { ?>
                    <div class="lesson-empty">No active subject assignments matched the selected class, batch, and semester yet. Assign the teacher to the subject first, then the lesson timetable can be created safely.</div>
                    <?php } ?>
                </div>
            </div>
        </section>
                        ...
                        <?php if(count($slotRows) > 0){ ?>
                            ...
                            <article class="lesson-entry-card" style="--lesson-accent: <?php echo lesson_timetable_escape($tokens['accent']); ?>; --lesson-soft: <?php echo lesson_timetable_escape($tokens['soft']); ?>; --lesson-surface: <?php echo lesson_timetable_escape($tokens['surface']); ?>; --lesson-ink: <?php echo lesson_timetable_escape($tokens['ink']); ?>;">
                                ...
                                <div class="lesson-entry-card__actions lesson-print-hide">
                                    <a class="lesson-chip-btn" href="lesson-timetable.php?batchid=<?php echo urlencode($row['batchid']); ?>&termname=<?php echo urlencode($row['termname']); ?>&classid=<?php echo urlencode($row['classid']); ?>&edit_lesson=<?php echo urlencode($row['lessonid']); ?>"><i class="fa fa-edit"></i> Edit</a>
                                    <a class="lesson-chip-btn lesson-chip-btn--danger" href="lesson-timetable.php?batchid=<?php echo urlencode($row['batchid']); ?>&termname=<?php echo urlencode($row['termname']); ?>&classid=<?php echo urlencode($row['classid']); ?>&delete_lesson=<?php echo urlencode($row['lessonid']); ?>" onclick="return confirm('Delete this lesson slot?');"><i class="fa fa-trash"></i> Delete</a>
                                </div>
                            </article>
                            <?php } ?>
                        <?php } else { ?>
